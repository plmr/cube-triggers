...
export class AlgorithmParserService {
  ...
  private parseAlgorithmLine(line: string): ParsedAlgorithm | null {
    ...
    const algType = this.classifyAlgorithm(caseName, normalizedMoves);
  ...
  private countMoves(normalizedMoves: string): number {
    ...
    return normalizedMoves.split(/\s+/).filter(move => move.length > 0).length;
  }

  /**
   * Classify algorithm type based on case name and moves
   */
  private classifyAlgorithm(caseName?: string, moves?: string): AlgType {
    ...
    // PLL patterns
    if (name.includes('pll') || name.includes('permutation') || 
        name.includes('perm') || name.includes('t-perm') || 
        name.includes('a-perm') || name.includes('u-perm')) {
      return AlgType.PLL;
    }
   ...
   */
  extractNgrams(normalizedMoves: string, minLength: number = 4, maxLength: number = 6): string[] {
    if (!normalizedMoves) return [];

    const moves = normalizedMoves.split(/\s+/).filter(move => move.length > 0);
    const ngrams: string[] = [];
