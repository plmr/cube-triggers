...
export class JobQueueService {
  ...
  async queueImportProcessing(data: ProcessImportJobData): Promise<string> {
    ...
    this.logger.log(
      `Queued import processing job ${job.id} for import ${data.importRunId}`,
    );
    return job.id!;
  }
   ...
   */
  async queueAggregateComputation(
    data: ComputeAggregatesJobData,
  ): Promise<string> {
    ...
    this.logger.log(
      `Queued aggregate computation job ${job.id} for import ${data.importRunId}`,
    );
    return job.id!;
  }
